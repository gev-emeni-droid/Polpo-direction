...
const allowedOrigins = new Set([
  ...
  "https://c01a07a9.polpo-direction.pages.dev",
  "https://f9ba74fb.polpo-direction.pages.dev",
  "http://localhost:5173",
  "http://localhost:8787",
]);
  ...
  try {
    ...
    const result = await c.env.DB.prepare(`
      INSERT INTO employees (id, first_name, last_name, display_name, role_id, status, 
                              ...
                              email, phone)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(id, first_name, last_name, display_name || `${first_name} ${last_name}`, role_id, status, 
            contract_hours_week || null, contract_type || null, is_external || 0, external_category || null,
            email || null, phone || null).run();
...
app.post('/api/shifts', async (c) => {
  try {
    ...
    const employeeExists = await c.env.DB.prepare('SELECT id FROM employees WHERE id = ?').bind(employee_id).first();
    if (!employeeExists) {
    ...
    const shiftResult = await c.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).bind(id, employee_id, date, role_id || null, notes || null).run();
...
app.post('/api/absences', async (c) => {
  try {
    ...
    // Vérifier que le code horaire existe
    const codeExists = await c.env.DB.prepare('SELECT code FROM shift_codes WHERE code = ?').bind(code).first();
    if (!codeExists) {
      return c.json(error('Shift code not found'), 400);
    }
