...
const apiCall = async (endpoint: string, options: RequestInit = {}) => {
  try {
    ...
    // Ne pas lancer d'erreur pour permettre le fallback localStorage
    throw error;
  }
};

// ===== ROLES =====
export const getRoles = async (): Promise<{id: string, label: string, header_bg_color?: string, sort_order?: number, is_active?: boolean, slug?: string}[]> => {
  try {
    ...
    if (fallbackRoles) {
      try {
        const parsed = JSON.parse(fallbackRoles);
        return Array.isArray(parsed) ? parsed : [];
      } catch {
        // Retourner données par défaut
        return [
          { id: 'encadrement', label: 'ENCADREMENT' },
          { id: 'commerciale-admin', label: 'COMMERCIALE + ADMIN' },
    ...
    // Retourner données par défaut si aucun fallback
    return [
      { id: 'encadrement', label: 'ENCADREMENT' },
      { id: 'commerciale-admin', label: 'COMMERCIALE + ADMIN' },
      { id: 'accueil', label: 'ACCUEIL' },
...
export const saveRoles = async (roles: {id: string, label: string, header_bg_color?: string, sort_order?: number, is_active?: boolean, slug?: string}[]): Promise<void> => {
  try {
...
export const addRole = async (role: {id: string, label: string, header_bg_color?: string}): Promise<{id: string, label: string, header_bg_color?: string}> => {
  try {
...
export const getEmployees = async (): Promise<Employee[]> => {
  try {
    ...
    if (fallbackEmployees) {
      try {
        const parsed = JSON.parse(fallbackEmployees);
        return Array.isArray(parsed) ? parsed : [];
      } catch {
        // Retourner données par défaut pour éviter l'erreur
        return [
          { id: '1', name: 'Jean Dupont', role: 'managers', isActive: true, isExternal: false, externalCategory: '', contractHoursWeek: 35, contractType: 'CDI', email: '', phone: '' },
          { id: '2', name: 'Marie Martin', role: 'barman', isActive: true, isExternal: false, externalCategory: '', contractHoursWeek: 35, contractType: 'CDI', email: '', phone: '' },
          { id: '3', name: 'Pierre Durand', role: 'chef-de-rang', isActive: true, isExternal: false, externalCategory: '', contractHoursWeek: 35, contractType: 'CDI', email: '', phone: '' },
          { id: '4', name: 'Sophie Lefebvre', role: 'accueil', isActive: true, isExternal: false, externalCategory: '', contractHoursWeek: 35, contractType: 'CDI', email: '', phone: '' },
          { id: '5', name: 'Thomas Bernard', role: 'apprenti', isActive: true, isExternal: false, externalCategory: '', contractHoursWeek: 35, contractType: 'CDI', email: '', phone: '' }
        ];
      }
    }
    // Retourner données par défaut si aucun fallback
...
export const getShiftCodes = async (): Promise<any[]> => {
  try {
    ...
    if (fallbackShiftCodes) {
      try {
        const parsed = JSON.parse(fallbackShiftCodes);
        return Array.isArray(parsed) ? parsed : [];
      } catch {
        // Retourner données par défaut
        return [
          { code: 'MIDI', label: 'Service Midi', defaultColor: '#3b82f6', defaultStartMidi: '11:30', defaultEndMidi: '15:00', isAbsence: false, isRest: false },
          { code: 'SOIR', label: 'Service Soir', defaultColor: '#8b5cf6', defaultStartSoir: '18:30', defaultEndSoir: '23:00', isAbsence: false, isRest: false },
          { code: 'RN', label: 'Repos Nuit', defaultColor: '#10b981', isAbsence: false, isRest: true },
          { code: 'AA', label: 'Absence Accident', defaultColor: '#ef4444', isAbsence: true, isRest: false },
          { code: 'CP', label: 'Congés Payés', defaultColor: '#f59e0b', isAbsence: true, isRest: false }
        ];
      }
    }
    // Retourner données par défaut si aucun fallback
...
export const initMockData = async (): Promise<void> => {
  try {
    ...
    // Toujours essayer l'API en premier pour utiliser les données de la DB
    console.log('📡 Trying to load data from your database...');
    ...
    try {
      ...
      // Sauvegarder les données de l'API dans localStorage pour le cache
      localStorage.setItem('polpo_employees', JSON.stringify(employees));
      localStorage.setItem('polpo_roles', JSON.stringify(roles));
      localStorage.setItem('polpo_shift_codes', JSON.stringify(shiftCodes));
      
    } catch (apiError) {
      console.error('❌ API failed, checking localStorage cache:', apiError);
      
      // Vérifier si on a des données en cache
      const cachedEmployees = localStorage.getItem('polpo_employees');
      const cachedRoles = localStorage.getItem('polpo_roles');
      const cachedShiftCodes = localStorage.getItem('polpo_shift_codes');
      
      if (cachedEmployees && cachedRoles && cachedShiftCodes) {
        console.log('✅ Using cached data from localStorage');
      } else {
        console.log('⚠️ No cached data found, you need to:');
        console.log('1. Fix CORS issues on your API server');
        console.log('2. Or add CORS headers to allow access from Cloudflare Pages');
        console.log('3. Your API needs to send: Access-Control-Allow-Origin: *');
        
        // Créer des données minimales pour éviter l'erreur
        const minimalRoles = [{ id: 'default', label: 'DEFAULT' }];
        const minimalEmployees = [{ id: '1', name: 'Employee', role: 'default', isActive: true }];
        const minimalShiftCodes = [{ code: 'MIDI', label: 'MIDI', defaultColor: '#3b82f6' }];
        
        localStorage.setItem('polpo_roles', JSON.stringify(minimalRoles));
        localStorage.setItem('polpo_employees', JSON.stringify(minimalEmployees));
        localStorage.setItem('polpo_shift_codes', JSON.stringify(minimalShiftCodes));
      }
    }
